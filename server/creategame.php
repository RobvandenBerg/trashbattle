...
if($gamedata -> {'maxwins'})
{
	...
	if($maxwins != 1 && $maxwins != 3 && $maxwins != 5 && $maxwins != 10 && $maxwins != 15)
	{
		$maxwins = 5;
	}
}
...
$connection = @fsockopen($host, $port);
	...
	else
	{
		...
		$insert_request = $mysqli -> query('INSERT into games (host,port,pid,max_players,guests,chat,maxcoins,maxwins,items,difficulty,password) VALUES ('.$userid.','.$port.','.$pid.','.$maxplayers.','.$guests.','.$chat.','.$maxcoins.','.$maxwins.',\''.$items.'\',"'.$difficulty.'","'.$password.'")') or die($mysqli -> error);
...
function launchNodeServer($port)
{
	...
	$cmd = '/usr/local/bin/node game.js ' . $port;
	exec("$cmd 1>logs/outlog_" . $port . ".txt 2>logs/errorlog_" . $port . ".txt & echo $!", $output);
	$pid = $output[0];
	return $pid;
}
