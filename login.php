...
if($_POST['login']) {
	...
	$check_login_request = $mysqli -> query('SELECT id,username,password,activated,moderator,god,usernameColor from users where username=\''.$mysqli -> real_escape_string($username).'\'') or die($mysqli -> error);
